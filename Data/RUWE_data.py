...
tstart=time.time()
mag_res=0.1; col_res=0.5; pos_level=7
#mag_res=1; col_res=1; pos_level=9
sample = 'ruwe1p4'
if False:
    version="gaiaedr3";
    ...
    with h5py.File(f'/data/asfe2/Projects/astrometry/gaiaedr3_{sample}_kncounts.h', 'w') as hf:
...
if True:
    ...
    query=f"""select (phot_g_mean_mag/{mag_res})::integer as magnitude,
                   ...
                   count(*) as n,
                   count(*) filter (where ruwe < 1.4 and astrometric_params_solved>3) as k
                   from gaia_edr3.gaia_source
                   where phot_g_mean_mag is not NULL
                   group by magnitude, colour, position
                   """

    table = f"gaiaedr3_{sample}_kncounts_Gres{1/mag_res:.0f}CRres{1/col_res:.0f}hpx{pos_level}"
