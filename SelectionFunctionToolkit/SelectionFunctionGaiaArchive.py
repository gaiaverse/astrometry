class GaiaArchiveQuery:
    ...
    def _build_query(self):
        ...
        subquery_line = ', \n'.join([f"gaia_healpix_index({v['resolution']}, {v['column']}) as {k}" if k == 'position' else f"to_integer(floor({v['column']} * {int(1.0/v['resolution'])})) as {k}" for k,v in self.request.items()])

        self.query = f"""select {select_line}, count(*) as n, sum(selection) as k \n from ( select {self.limit_results} {subquery_line}, \n{subset_line} from {self.gaia_version}.gaia_source \n {superset_line} {self.random_results} ) as subquery \n group by {select_line}"""
